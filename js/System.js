class System {
  ...
  addProduct(id,title,description,imgUrl,price,stock,offer,state,selled) {
    let newProduct = new Product(id,title,description,imgUrl,price,stock,offer,state, selled)
  ...
  buyProduct(productId, amount) {
    console.log("Intentando comprar producto con ID:", productId);
    console.log("Productos antes de comprar:");
    console.log(this.products.map(prod => prod.id)); // Imprimir solo los IDs de los productos
  
    const productIdNumber = parseInt(productId); // Convertir el ID a número
    const product = this.products.find(prod => prod.id === productIdNumber);
    ...
    console.log(`Compra realizada: ${amount} ${product.name} por ${totalCost} USD`);
  
    product.stock -= amount;
    product.selled = true; // Marca el producto como vendido
  
    const newBuy = new Buys(this.buys.length + 1, product, amount, true);
